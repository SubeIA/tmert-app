...
export const createUser = onCall(async (request: CallableRequest<CreateUserData>) => {
  ...
  const callerDoc = await admin.firestore().collection('users').doc(context.uid).get();
...
export const updateUser = onCall(async (request: CallableRequest<UpdateUserData>) => {
  const data = request.data;
  const context = request.auth;

  if (!context) {
    throw new HttpsError('unauthenticated', 'Debes estar autenticado para actualizar usuarios');
  }
  ...
  const callerRole = callerDoc.data()?.role;
  if (callerRole !== 'admin') {
    throw new HttpsError('permission-denied', 'Solo administradores pueden actualizar usuarios');
  }
  ...
  try {
    ...
    throw new HttpsError('internal', error.message || 'Error al actualizar usuario');
  }
});

export const deleteUser = onCall(async (request: CallableRequest<{ userId: string }>) => {
  const data = request.data;
  const context = request.auth;

  if (!context) {
    throw new HttpsError('unauthenticated', 'Debes estar autenticado para eliminar usuarios');
  }
